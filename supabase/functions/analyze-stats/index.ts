...
serve(async (req) => {
  ...
  try {
    ...
    const { statsPayload } = await req.json();
    if (!statsPayload) {
...
2. **Fortalezas** — Qué está funcionando bien (jugadores destacados, zonas calientes, eficiencia).
...
5. **Jugadores destacados** — Quién merece atención especial (positiva o por desarrollo).
